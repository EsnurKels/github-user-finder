...
export const ProfileCard = ({ user, repos }: ProfileCardProps) => {
  // Dil Analizi Hesaplama (App.tsx'ten buraya taşıdık)
  ...
  return (
    <div className="lg:col-span-1 bg-light-card dark:bg-dark-card p-6 rounded-3xl shadow-xl border border-light-border dark:border-dark-border h-fit flex flex-col gap-4 overflow-y-auto custom-scrollbar">
      <div className="flex flex-col items-center text-center">
        <img src={user.avatar_url} className="w-24 h-24 rounded-full border-4 border-light-accent dark:border-dark-accent mb-3 shadow-xl" alt="avatar" />
              ...
              {languages.map((lang) => (
                <div key={lang.name}>
                  ...
                  <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full bg-light-accent dark:bg-dark-accent transition-all duration-1000" style={{ width: `${lang.percent}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <a href={user.html_url} target="_blank" rel="noreferrer" className="w-full bg-light-accent dark:bg-dark-accent text-white py-3 rounded-xl font-bold text-sm mt-4">GitHub Profilini Aç</a>
      </div>
    </div>
  );
};