  ...
  return (
    <div className="lg:col-span-2 flex flex-col overflow-hidden min-h-0">
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-4 pb-4">
        {repos.map((repo) => (
          <div key={repo.id} className="bg-light-card dark:bg-dark-card p-5 rounded-2xl border border-light-border dark:border-dark-border shadow-md hover:shadow-xl hover:-translate-y-1 transition-all group h-fit">
            <h4 className="font-bold text-md mb-2 group-hover:text-light-accent dark:group-hover:text-dark-accent transition-colors truncate">{repo.name}</h4>
            ...
            <div className="flex justify-between items-center mt-auto">
              ...
              <a href={repo.html_url} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-light-accent dark:text-dark-accent hover:underline">İncele →</a>
            </div>
          </div>
        ))}
      </div>
